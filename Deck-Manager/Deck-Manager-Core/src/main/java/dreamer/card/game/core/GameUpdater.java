    ...
    public void updateLocal() {
        ...
        try {
            if (db != null && db.exists()) {
                ...
                dbProperties.put(PersistenceUnitProperties.JDBC_URL, "jdbc:h2:file:" + db.getParentFile().getAbsolutePath() + System.getProperty("file.separator") + dbName + ";AUTO_SERVER=TRUE");
                        ...
                        if (!dbError) {
                            ...
                            for (Iterator it2 = set.getCards().iterator(); it2.hasNext();) {
                                ...
                                if (!Lookup.getDefault().lookup(IDataBaseCardStorage.class).cardExists(card.getName())) {
                                    ...
                                    card = Lookup.getDefault().lookup(IDataBaseCardStorage.class).createCard(cardType, card.getName(), ((Card) card).getText());
