...
public final class ICardSetNode extends BeanNode
{
  ...
  private class UpdateSetAction extends AbstractAction
  {
    public UpdateSetAction()
    {
      super("Update");
    }
    ...
    {
      for (GameUpdater updater : Lookup.getDefault().lookupAll(GameUpdater.class))
      {
        if (updater.getGame().getName().equals(set.getCardGame().getName()))
        {
          updater.updateSet(set);
          break;
        }
      }
    }
